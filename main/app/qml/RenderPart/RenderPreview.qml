    ...
    */
    ...
    Timer
    {
        ...
        onTriggered:
        {
            ...
            // Update rotation angle based on current speed. Modulo 360 to keep it in a reasonable range.
            // squaresRepeater.opacity = 0.5;
            // root.rotationAngle = (root.rotationAngle + (root.currentSpeedDegPerSecond * interval / 1000.0)) % 360;
            // squaresRepeater.opacity = 0.75;
            // root.rotationAngle = (root.rotationAngle + (root.currentSpeedDegPerSecond * interval / 1000.0)) % 360;
            // squaresRepeater.opacity = 1;
            root.rotationAngle = (root.rotationAngle + (root.currentSpeedDegPerSecond * interval / 1000.0)) % 360;
        }
    }
    ...
    ColumnLayout
    {
        ...
        Rectangle
        {
            ...
            Item
            {
                ...
                Item
                {
                    ...
                    Row
                    {
                        ...
                        id: squaresRow
                        spacing: Math.max(0, renderItem.ledDistance)

                        Repeater
                        {
                            id: squaresRepeater
                            model: Math.max(1, renderItem.ledCount)
                            Rectangle
                            {
                                id: qml_wrapper
                                width: Math.max(1, renderItem.ledSize)
                                height: Math.max(1, renderItem.ledSize)
                                color: "red"
                            }
                        }
                    }
            ...
            Row
            {
                ...
                Label {text: "Speed: " + Math.round(root.currentSpeedDegPerSecond) + "°/s"}
                Label {text: "Blur: " + Math.round(Math.min(64, root.blurBase + Math.abs(root.currentSpeedDegPerSecond) / 4))}
                Slider
                {
                    id: speedSlider
    ...
    Connections
    {
         ...
         function onTransformReadyForQml()
         {
             ...
             console.log("RenderPreview image size:",
                         transform_engine.transformedWidth,
                         "x",
                         transform_engine.transformedHeight);
             renderItem.requestRepaint();
         }
     }
