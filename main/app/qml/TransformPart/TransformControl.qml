...
Control {
    ...
    function applySettings(settings) {
        ...
        l_radius.setCurrentValue(clampInt(settings.radius !== undefined ? settings.radius : l_radius.init_value, l_radius.min, l_radius.max));
        ang_res.setCurrentValue(clampInt(settings.angularResolution !== undefined ? settings.angularResolution : ang_res.init_value, ang_res.min, ang_res.max));
        l_load_zoom.setCurrentValue(clampInt(settings.loadZoom !== undefined ? settings.loadZoom : l_load_zoom.init_value, l_load_zoom.min, l_load_zoom.max));
        l_out_zoom.setCurrentValue(clampInt(settings.outputZoom !== undefined ? settings.outputZoom : l_out_zoom.init_value, l_out_zoom.min, l_out_zoom.max));
        onRadiusUpdate();
        onAngResUpdate();
        onZoomLoadUpdate();
        onZoomOutUpdate();
    ...
    contentItem: ColumnLayout {
        ...
        GroupBox {
            ...
            contentItem: ColumnLayout {
                ...
                Text {
                    text: "Size of loaded img [pixels]: "
                    font.bold: true
                    font.pixelSize: 18
                }
                Text {
                    id: loadedImgSizeText
                    text: "0 x 0"
                    font.pixelSize: 18
                }
                Text {
                    text: "Loaded img preview zoom"
                    font.bold: true
                    font.pixelSize: 18
                }
                DataInput {
                ...
                Text {
                    text: "Transformed image size: "
                    font.bold: true
                    font.pixelSize: 18
                }
                TextField {
                    readOnly: true
                    text: transform_engine && transform_engine.transformedWidth > 0 && transform_engine.transformedHeight > 0 ? transform_engine.transformedWidth + " x " + transform_engine.transformedHeight + " pixels" : "-"
                    color: "black"
                    font.pixelSize: 20
